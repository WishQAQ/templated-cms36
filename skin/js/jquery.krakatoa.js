...
(function($) {
	...
	$.fn.krakatoa = function(options) {
		...
		return this.each(function() {
			...
			slider.find('.krakatoa-control').append('<ul class="arrows">' +
				'<li data-move="-1" class="arrow arrow-left"><a href="javascript:void(0)">&laquo;</a></li>' +
				'<li data-move="1" class="arrow arrow-right"><a href="javascript:void(0)">&raquo;</a></li>' +
				'</ul>');
			...
			if (settings.buttons) {
				...
				for (i = 0; i < length / settings.items; i++) {
					buttons += '<li class="pagination"><a href="javascript:void(0)">' + i + '</a></li>';
				}
				buttons += '</ul>';
			...
			if (settings.topbuttons) {
				topbuttons = '<ul class="topbuttons">';
				for (i = 0; i < length / settings.items; i++) {
					topbuttons += '<li class="toppagination"><a href="javascript:void(0)">' + i + '</a></li>';
				}
				topbuttons += '</ul>';
				slider.find('.krakatoa-control').append(topbuttons);
				slider.find('.topbuttons').on('click touchstart', 'li', {settings: settings}, do_the_move )
					  .find('li').eq(settings.first).addClass('topactive-button');
			}
	...
	function do_the_move(e) {
		...
		if (self.attr('data-move')) { // arrow or auto play
		 ...
		 }else if (settings.topbuttons) { // button
			if (self.hasClass('topactive-button')) return;
			move = (self.index() * settings.items > current) ? 1 : -1;
			next = self.index() * settings.items;
		}
		...
		if (settings.topbuttons) {
			slider.find('.topactive-button').removeClass('topactive-button')
				  .parent().children().eq(current).addClass('topactive-button');
		}	}
